...
class MailService
{
    ...
    public function sendOrderMail(Order $Order)
    {
        ...
        $message = (new \Swift_Message())
        ->setSubject('['.$this->BaseInfo->getShopName().'] '.$MailTemplate->getMailSubject())
        ->setFrom(['goods@s-t-s-k.com' => $this->BaseInfo->getShopName()])
        ->setTo([$Order->getEmail()])
        ->setBcc('goods@s-t-s-k.com')
        ->setReplyTo('goods@s-t-s-k.com')
        ->setReturnPath('goods@s-t-s-k.com');
     ...
     * @return \Swift_Message
     */
    public function sendCstmOrderMail($Customer, $Order, $additionalAddress, $CustomerAddress)
    {
        log_info('受注メール送信開始');
...
//         $count = $this->mailer->send($message);
    ...
    /**
     * Send sample order mail.
     *
     * @return \Swift_Message
     */
    public function sendSampleOrderMail($data)
    {
        log_info('受注メール送信開始');
        ...
        $body = $this->twig->render($MailTemplate->getFileName(), [
            'data' => $data,
            'totalPrice' => intval($data['quantity']) * $data['price'] + 800
        ]);
