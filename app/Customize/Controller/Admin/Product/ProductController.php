...
class ProductController extends AbstractController
{
    ...
    public function index(Request $request, $page_no = null, PaginatorInterface $paginator)
    {
        ...
        if ($this->productRepository->findOneBy(['name' => '人気ランキング']) != null) {
            $ProductRank = $this->productRepository->findOneBy(['name' => '人気ランキング'])->getDescriptionDetail();
            $temp = explode(';', $ProductRank);
    
            foreach($temp as $item) {
              if (count(explode(':', $item)) == 2)
                array_push($rank, intval( explode(':', $item)[1] ));
              else array_push($rank, 0);
            }
        }
    ...
    public function edit(Request $request, $id = null, RouterInterface $router, CacheUtil $cacheUtil)
    {
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                $template_file = $form->get('template_file')->getData();
                if ( $template_file !== null ) {
                    $extension = $template_file->getClientOriginalExtension();
                    $filename = $template_file->getClientOriginalName();
                ...
                for ($i = 0; $i < count($price_range) - 1; $i++) {
                    if ($Product->getPrice02Min() >= $price_range[$i] && $Product->getPrice02Min() < $price_range[$i + 1]) {
                ...
                $flagToAddPrice = true;
                foreach($Product->getProductCategories() as $ProductCategory)
                    if ($ProductCategory->getCategory()->getName() == $price_category_name) {
                        $flagToAddPrice = false;
                        break;
                    }
                if ($flagToAddPrice) {
    ...
    /**
     * @Route("/%eccube_admin_route%/rank", name="admin_rank")
     */
    public function setRankProducts(Request $request)
    {
        $rank = $_POST['rank'];
        $id = $_POST['id'];

        $tempProduct = $this->productRepository->findOneBy(['name' => '人気ランキング']);
        $rankString = $tempProduct->getDescriptionDetail();

        $left = substr($rankString, 0, strpos($rankString, $rank.':') + 2 );

        if ($rank == 4) $right = '';
        else $right = substr($rankString, strpos($rankString, ';'.($rank + 1)) );

        $tempProduct->setDescriptionDetail($left.$id.$right);
        $this->entityManager->persist($tempProduct);
        $this->entityManager->flush($tempProduct);

        $tempProduct = $this->productRepository->find($id);
        $image = $tempProduct->getMainListImage() == null ? 'no_image_product.png' : $tempProduct->getMainListImage()->getFileName();

        $response = [ 'name' => $tempProduct->getName(), 
            'image' => $image,
            'product_code' => $tempProduct->getCodeMax()
        ];
